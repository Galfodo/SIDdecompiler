...
// This is intended as a test program for SASMemu. It compares the memory and CPU state of SASMemu with that of Cadaver's siddump emulator.
...
static bool
compareMem(SASM::C64MachineState& emu, unsigned char* m) {
  if (memcmp(emu.m_Mem, m, 65536) == 0) {
    if (pc != emu.PC) {
      return false;
    }
    if (a != emu.A) {
      fprintf(stderr, "Register error: A : $%02x, should be $%02x. PC : $%04x\n", (int)emu.A, (int)a, (int)emu.PC);
      goto error;
    }
    if (x != emu.X) {
      fprintf(stderr, "Register error: X : $%02x, should be $%02x. PC : $%04x\n", (int)emu.X, (int)x, (int)emu.PC);
      goto error;
    }
    if (y != emu.Y) {
      fprintf(stderr, "Register error: Y : $%02x, should be $%02x. PC : $%04x\n", (int)emu.Y, (int)y, (int)emu.PC);
      goto error;
    }
    if (flags != emu.SR) {
      fprintf(stderr, "Register error: SR : $%02x, should be $%02x. PC : $%04x\n", (int)emu.SR, (int)flags, (int)emu.PC);
      goto error;
    }
    if (cpucycles != emu.m_Debugger.m_CPUCycles) {
      fprintf(stderr, "Register error: CPUCYCLES : %d, should be %d. PC : $%04x\n", (int)emu.m_Debugger.m_CPUCycles, (int)cpucycles, (int)emu.PC);
      goto error;
    }
    return true;
  }
error:
  int offset = 0;
  for (;offset < 65536; ++offset) {
    if (emu.m_Mem[offset] != m[offset]) {
      printf("Memory error at $%04x : $%02x, should be $%02x\n", offset, (int)emu.m_Mem[offset], (int)m[offset]);
      break;
    }
  }
  exit(-1);
...
int main(int argc, char **argv)
{
  ...
  SASM::MinimalCycleCountingEmu
    emu;
  
  fread(&emu.m_Mem[loadaddress], loadsize, 1, in);
  memcpy(mem, emu.m_Mem, 65536);
  ...
  if (playaddress == 0)
  {
    ...
    if ((emu.m_Mem[0x01] & 0x07) == 0x5)
      playaddress = emu.m_Mem[0xfffe] | (emu.m_Mem[0xffff] << 8);
    else
      playaddress = emu.m_Mem[0x314] | (emu.m_Mem[0x315] << 8);
  ...
  while (frames < firstframe + seconds*50)
  {
    ...
    while (emu.runCPU() && runcpu())
    {
      ...
      if ((emu.m_Mem[0x01] & 0x07) != 0x5 && (emu.PC == 0xea31 || emu.PC == 0xea81))
        break;
    }

    //unsigned char* m = mem;
    unsigned char* m = emu.m_Mem;

    // Get SID parameters from each channel and the filter
    for (c = 0; c < 3; c++)
    {
      chn[c].freq   = m[0xd400 + 7*c]   | (m[0xd401 + 7*c] << 8);
      chn[c].pulse  = (m[0xd402 + 7*c]  | (m[0xd403 + 7*c] << 8)) & 0xfff;
      chn[c].wave   = m[0xd404 + 7*c];
      chn[c].adsr   = m[0xd406 + 7*c]   | (m[0xd405 + 7*c] << 8);
    }
    filt.cutoff     = (m[0xd415] << 5)  | (m[0xd416] << 8);
    filt.ctrl       = m[0xd417];
    filt.type       = m[0xd418];
    ...
    if (frames >= firstframe)
    {
      ...
      if (profiling)
      {
        int cycles = emu.debugger().m_CPUCycles;
